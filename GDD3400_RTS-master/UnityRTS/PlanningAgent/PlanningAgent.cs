...
namespace GameManager
{
	...
	public class PlanningAgent : Agent
	{
		...
		{
			if (state == AgentState.InitialBuild)
			{
				//OR conditions (only 1 needs to be true)
				float barrHeur = Mathf.Clamp(1 - myBarracks.Count, 0, 1);
				float barrVsOpp = Mathf.Clamp(enemyBarracks.Count - myBarracks.Count, 0, 1);
				float workerCount = Mathf.Clamp(maxWorkerCount - myWorkers.Count, 0, 1);

				//These are required criteria 
				float underMaxBarrCount = Mathf.Clamp(maxBarracksCount - myBarracks.Count, 0, 1);
				float goldCheck = Mathf.Clamp(Gold - Constants.COST[UnitType.BARRACKS], 0, 1);

				return Mathf.Clamp((barrHeur + barrVsOpp) * goldCheck * underMaxBarrCount, 0, 1);
			}
			else if (state == AgentState.ArmyBuilding)
			{
				float barrVsOpp = Mathf.Clamp(enemyBarracks.Count - myBarracks.Count, 0, 1);
				float armyVsOpp = Mathf.Clamp((enemyArchers.Count + enemySoldiers.Count) - (mySoldiers.Count + myArchers.Count), 0, 1);
				float goldCheck = Mathf.Clamp(Gold - Constants.COST[UnitType.BARRACKS], 0, 1);

				return Mathf.Clamp((barrVsOpp + armyVsOpp) * goldCheck, 0, 1);
			}
			...
			{
				Debug.LogError("No State for Barracks Heuristic");
				return 0;
			}
		}
		...
		private float CalcArmyHeur()
		{
			...
			else if (state == AgentState.ArmyBuilding)
			{
				...
				float armyHeur = Mathf.Clamp(targetArmySize - mySoldiers.Count + myArchers.Count, 0, 1);
				float armyVsOpp = Mathf.Clamp((enemyArchers.Count + enemySoldiers.Count) + 5 - (mySoldiers.Count + myArchers.Count), 0, 1);
				float goldCheck = Mathf.Clamp(Gold - Constants.COST[UnitType.SOLDIER], 0, 1);
			...
			else if (state == AgentState.Attacking)
			{
				...
				return Mathf.Clamp(armyVsOpp * goldCheck, 0, 1);
			}
			else
			{
				Debug.LogError("No State for Army Heuristic");
				return 0;
			}
		}
		...
		private float CalcAttackHeur()
		{
			...
			{
				return Mathf.Clamp((myArchers.Count + mySoldiers.Count) - (enemySoldiers.Count + enemyArchers.Count) + 5, 0, 1);
			}
			else if (state == AgentState.Attacking)
			{
				return Mathf.Clamp((myArchers.Count + mySoldiers.Count) - (enemySoldiers.Count + enemyArchers.Count), 0, 1);
			...
			{
				Debug.LogError("No State for Attacking Heuristic");
				return 0;
			}
		...
		private float GatherGoldHeur()
		{
			if (state == AgentState.InitialBuild)
			{
				float targetGold = 750;
				return Mathf.Clamp(targetGold - Gold, 0, 1);
			}
			else if (state == AgentState.ArmyBuilding)
			{
				float targetGold = Constants.COST[UnitType.SOLDIER] * 2;
				return Mathf.Clamp(targetGold - Gold, 0, 1);
			}
			else if (state == AgentState.Attacking)
			{
				float targetGold = Constants.COST[UnitType.SOLDIER];
				return Mathf.Clamp(targetGold - Gold, 0, 1);
			}
			else
			{
				Debug.LogError("No State for Gathering Heuristic");
				return 0;
			}
		}
		...
		private float CalcRefHeur()
		{
			...
			{
				float refHeur = Mathf.Clamp(2 - myRefineries.Count, 0, 1);
				float goldCheck = Mathf.Clamp(Gold - Constants.COST[UnitType.REFINERY], 0, 1);
			...
			{
				float refHeur = Mathf.Clamp(3 - myRefineries.Count, 0, 1);
				float goldCheck = Mathf.Clamp(Gold - Constants.COST[UnitType.REFINERY], 0, 1);
			...
			{
				Debug.LogError("No State for Refinery Heuristic");
				return 0;
			}
		}
		#endregion
		...
		private void TrainWorkers()
		{
			...
			{
				Unit Base = GameManager.Instance.GetUnit(BaseNum);
				if(Base != null && Base.IsBuilt && Base.CurrentAction == UnitAction.IDLE)
				{
					Train(Base, UnitType.WORKER);
				}
			}
		}
		...
		private void BuildABuilding(UnitType BuildingType)
		{
